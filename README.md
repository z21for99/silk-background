...
`dsh` Web GUI 的客户端插件：WebGL "Silk" 丝绸着色器动态背景 + 全站玻璃化。
基于官方 web-profile 插件机制（`dsh.client` 声明 + bundle patch 层），
不修改 dsh 任何源码或前端产物。

## 特性

- **WebGL Silk 着色器**（21st.dev Shader Builder "Silk"）：全屏动态丝绸纹理，
  指针跟随/扭曲、自适应分辨率与像素预算、页面不可见时自动停渲染
- **三模式**：`off` / `dim`（默认，可读性优先的玻璃）/ `vivid`（高亮丝绸），
  右下角圆形按钮一键循环切换，选择持久化在 `localStorage`
- **玻璃化**：通过 dsh 官方 theme 服务的 `overrideTokens` 接口注册 alias token
  覆盖层（`--dsw-alias-bg-base`、`--dsw-alias-bg-layer-1/2`、
  `--dsw-specific-sidebar-fill`，亮/暗两套值），让应用自己的 CSS 变半透明，
  无需逐元素 DOM 手术，主题重同步与亮暗切换都自动保持（v4）
- **设置面板**：独立 ⚙ 按钮打开；速度、强度滑杆，7 套配色
  （紫黑/默认/原版/绿/蓝/青/红），4 色 RGB 取色板；全部持久化
- **健壮性**：`prefers-reduced-motion` 适配、WebGL 上下文丢失/恢复、
  主题重同步自愈、侧栏渐变/阴影抛光
- **调试口**：URL 加 `?silk-debug` 打开状态面板；控制台可用 `window.__dshSilk`
